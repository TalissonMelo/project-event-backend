...
public class OrderService {
	...
	public void cpfCnpj(Integer event, Integer user) {
		User userNow = userService.findById(user);
		Event eventNow = eventService.findById(event);

		if (eventNow.getType().equals(EventType.PRIVATE)) {
			if (!eventNow.getParticipants().contains(userNow.getCpfCnpj())) {
				throw new ObjectNotFoundException("Você não esta na lista de Participantes");
			}
		}
